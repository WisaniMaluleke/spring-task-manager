...
![Java](https://img.shields.io/badge/Java_JDK_17+-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
...
## 🗺️ Monolithic Architecture Blueprint

This ecosystem operates as a classic full-stack monolith inside an isolated, private Docker bridge network (`wisgeek-network`). Because the JSP view layer is processed directly by the embedded web container (Tomcat) alongside your Spring controllers, front-end actions and back-end events execute within the same container context.
...
```text
               ...
               └─────────────┬─────────────┘
                             │
       ┌─────────────────────┼─────────────────────┐
       ▼ (Port 8040)         ▼ (Port 8081)         ▼ (Port 3000)
┌──────────────────────┐┌──────────────┐      ┌──────────────┐
│  Full-Stack Monolith ││   Adminer    │      │ Grafana GUI  │
│  [JSP Views + MVC]   ││ (Database UI)│      │ (Visuals/UI) │
└──────────┬───────────┘└──────┬───────┘      └──────▲───────┘
           │                   │                     │
           │ (Internal:5432)   │ (Internal:5432)     │ (Internal:3100)
           ▼                   ▼                     │
┌──────────────────────────────────────┐             │
│         PostgreSQL Database          │             │
...
Launch the environment in detached mode using your terminal panel:
...
Ensure all core services are marked as healthy and running:
...
The database schema seeds automatically on the first boot cycle via `init.sql`. To force an absolute database flush and reapply fresh mock table metrics, run:
...
## 📊 Monolithic Log Analytics (PLG Stack Setup)

Because frontend JSP rendering and server-side logic execute sequentially in the same container, you can trace user interactions and system faults in a single, cohesive log pipeline.
...
To configure a **Stat Panel** on your custom dashboards tracking your Spring application's error spike incidents inside a moving 5-minute window, use this formula:
